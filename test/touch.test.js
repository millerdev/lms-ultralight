...
describe('touch', function () {
  ...
  describe('TouchList', function () {
    it("should setup touch handlers on mount", function () {
      ...
      render(<mod.TouchList ref={ref} items={[1, 2, 3]} />)
      const instance = ref.current
      const listElement = { addEventListener: () => {}, removeEventListener: () => {} }
    ...
    function stateTest(name, perform, startConfig, endConfig) {
    ...
    describe("onItemSelected", function () {
      function test(startConfig, ix, endConfig, modifier) {
        const perform = touchlist => touchlist.onItemSelected(ix, modifier)
        stateTest("sel " + ix, perform, startConfig, endConfig)
      }
    ...
    describe("clearSelection", function () {
      function test(startConfig, endConfig) {
        const perform = touchlist => touchlist.clearSelection()
        stateTest("clear", perform, startConfig, endConfig)
      }
  ...
  describe("LoadingList", function () {
    ...
    it("with items and before should have space above", function () {
      renderWithHeight(10, {items: [1, 2], itemsOffset: 1}, list => {
        ...
        assert.equal(list[0].style.height, '5px')
      })
    })
    ...
    it("with items and after should have space below", function () {
      renderWithHeight(12, {items: [1, 2], itemsTotal: 3}, list => {
        assert.equal(list.length, 2)
        assert.equal(list[list.length - 1].style.height, '6px')
      })
    })
    ...
    it("with before and after should have space above and below", function () {
      ...
      renderWithHeight(8, props, list => {
        ...
        assert.equal(list[0].style.height, '4px')
        assert.equal(list[list.length - 1].style.height, '12px')
      })
    })
    ...
    function renderWithHeight(height, props, check) {
      let asserted = false
      rewire(module, {
        useResizeObserver: () => ({height, ref: () => {}}),
      }, () => {
        check(render(<mod.LoadingList {...props} />).container.children)
        asserted = true
      })
