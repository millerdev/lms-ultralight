...
- [ ] **T1** — Remove `import 'semantic-ui-css/semantic.min.css'` from `src/app.js`. Visual cliff; keep isolated so a regression is trivially revertable. QA pass before proceeding.
- [ ] **T2** — `npm uninstall semantic-ui-react semantic-ui-css`.
